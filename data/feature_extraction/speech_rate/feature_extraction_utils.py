...
def get_formant_attributes(sound, time_step=0., pitch_floor=75., pitch_ceiling=600.,
    ...
    attributes['f1_std'] = statistics.stdev(f1_list)
    attributes['f2_mean'] = statistics.mean(f2_list)
    attributes['f2_std'] = statistics.stdev(f2_list)
    attributes['f3_mean'] = statistics.mean(f3_list)
    attributes['f3_std'] = statistics.stdev(f3_list)
    attributes['f4_mean'] = statistics.mean(f4_list)
    attributes['f4_std'] = statistics.stdev(f4_list)
   ...
   #                            (7 * (35000 / (4 * attributes['f4_median'])))) / 4
    ...
    denominator_f1 = 4 * attributes['f1_median']
    denominator_f2 = 4 * attributes['f2_median']
    denominator_f3 = 4 * attributes['f3_median']
    denominator_f4 = 4 * attributes['f4_median']

    # Check if any denominator is zero before performing the division
    if denominator_f1 == 0 or denominator_f2 == 0 or denominator_f3 == 0 or denominator_f4 == 0:
        print('division by zero is handled!')
        # Handle the case where division by zero occurs
        # For example, set the result to a default value
        attributes['fitch_vtl'] = 0  # Set the result to 0
    else:
        # Perform the division
        attributes['fitch_vtl'] = ((1 * (35000 / denominator_f1)) +
                                   (3 * (35000 / denominator_f2)) +
                                   (5 * (35000 / denominator_f3)) +
                                   (7 * (35000 / denominator_f4))) / 4
